...
public class DefaultSpecialty implements SpecialtyUseCase {
    private final SpecialtyPersistent persistent;

    public DefaultSpecialty(SpecialtyPersistent persistent) {
    ...
    public List<Specialty> getSpecialties(Pagination pagination) {
        return persistent.getSpecialties(pagination);
    }
    ...
    public Specialty createSpecialty(Specialty spec) {
        ...
        if (Objects.nonNull(saved)) {
            throw new IllegalArgumentException(DUPLICATE_SPECIALTY);
        }
